...
@if($labors)
  ...
  {{ Form::open(array('route' => array('report.sub.labors.save.post', $reportId), 'class' => 'form-horizontal', 'role' => 'form')) }}

  @foreach($labors as $labor)

      <!-- Supplier -->
      <div class="form-group {{ $errors->first('supplier_id') ? 'has-error has-feedback' : '' }}">
        {{ Form::label('supplier_id['.$labor->id.']', trans("report.supplier_id"), array('class' => 'col-md-2 control-label')) }}
          <div class="col-md-10">
            <?php $report = Report::find($reportId); ?>
            {{ Form::select('supplier_id['.$labor->id.']', Supplier::getSuppliers('supply_and_install'), $labor->supplier_id ? $labor->supplier_id : null, array('class' => 'form-control')) }}
      ...
      <div class="form-group {{ $errors->first('num_labors') ? 'has-error has-feedback' : '' }}">
        {{ Form::label('num_labors['.$labor->id.']', trans("report.num_labors"), array('class' => 'col-md-2 control-label')) }}
          <div class="col-md-10">
            {{ Form::text('num_labors['.$labor->id.']', $labor->num_labors ? $labor->num_labors : null, array('class' => 'form-control', 'placeholder' => trans("report.placeholder.num_labors"))); }}
      ...
      <div class="form-group {{ $errors->first('skill') ? 'has-error has-feedback' : '' }}">
        {{ Form::label('skill['.$labor->id.']', trans("report.skill"), array('class' => 'col-md-2 control-label')) }}
          <div class="col-md-10">
            {{ Form::select('skill['.$labor->id.']', Occupation::getOccupations(), $labor->skill ? $labor->skill : null, array('class' => 'form-control')) }}
  ...
  @endforeach
    ...
    <!-- Supplier -->
    <div class="form-group {{ $errors->first('supplier_id') ? 'has-error has-feedback' : '' }}">
      {{ Form::label('supplier_id[]', trans("report.supplier_id"), array('class' => 'col-md-2 control-label')) }}
        <div class="col-md-10">
          {{ Form::select('supplier_id[]', Supplier::getSuppliers('supply_and_install'), null, array('class' => 'form-control')) }}
          @if($errors->first('supplier_id'))
            <span class="glyphicon glyphicon-remove form-control-feedback"></span>
          @endif
        </div>
    ...
    <div class="form-group {{ $errors->first('skill') ? 'has-error has-feedback' : '' }}">
      {{ Form::label('skill[]', trans("report.skill"), array('class' => 'col-md-2 control-label')) }}
        <div class="col-md-10">
          {{ Form::select('skill[]', Occupation::getOccupations(), null, array('class' => 'form-control')) }}
          @if($errors->first('skill'))
            <span class="glyphicon glyphicon-remove form-control-feedback"></span>
          @endif
        </div>
